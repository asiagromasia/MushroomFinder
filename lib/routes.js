module.exports = (app) => {
   ...
   app.get('/', (req,res, next) => {
       mushroom.find((err, mushrooms) => {
           console.log(mushrooms)
            console.log(mushroom.size);
           if (err) return next(err);
          res.type('text/html');
           res.render('home', {mushrooms: JSON.stringify(mushrooms)});
        //  res.render('home', {wantDisplay: true});
      });
   });
    ...
    //add new or update  
    //localhost:4000/api/v1/add       inserted via Postman
 ...
 /*   
    app.post('/api/v1/add', (req, res, next) => {
        if(!req.body._id){
          let newMush = new mushroom({name:req.body.name, size: req.body.size, location:req.body.location});  
            mushroom.save((err,newMush) =>{
                if (err) return next(err);
                console.log(newMush);
                res.json({updated: 0, _id: newMush._id});
            });
        } else {
            mushroom.updateOne({_id:req.body._id},{name:req.body.name, size: req.body.size, location:req.body.location}, (err, result) => {
                if (err) return next(err);
